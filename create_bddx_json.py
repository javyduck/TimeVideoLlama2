...
def get_car_info(car_info, predict_cs=False):
    ...
    # Context
    if predict_cs:
        final_cs = f'Speed: <t_start>{speed[-1]}<t_end> Course: <t_start>{course[-1]}<t_end>'
    ...
    context = f"The current video records driving scenario: <video>\n Control Signal until current Frame Sequence is: Speed: <t_start>{speed}<t_end>\n Curvature: <t_start>{curvature}<t_end>\n Acceleration: <t_start>{acceleration}<t_end>\n Course: <t_start>{course}<t_end>"
...
def main(split):
    ...
    for root, dirs, files in os.walk(info_root):
        for f in tqdm(files):
            ...
            with open(info_path,"r") as inf:
                ...
                info_dict = {
                    "id": f"{idx}",
                    "video": 'videos/' + video_path.split("/")[-1],
                    "conversations": new_conversation
                }
                if video_path.split("/")[-1] in ['10af6ba8-167c93c2_11906.mp4', '1036b362-92000280_11763.mp4', '22f325ef-1cafcfb4_21220.mp4', '231bdb08-2859bb11_21308.mp4']:
                    print("pass")
                    continue
    ...
    out_path = f'{out_dir}/time_conversation_bddx_{sp}.json'
