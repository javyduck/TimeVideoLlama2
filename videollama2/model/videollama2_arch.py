...
class Videollama2MetaForCausalLM(ABC):
    ...
    def get_embed_tokens(self, token_ids):
        embeddings = self.get_model().embed_tokens
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if (torch.any(torch.stack([cur_input_ids == MMODAL_TOKEN_INDEX[key.upper()] for key in keys]), dim=0)).sum() == 0:
                ...
                cur_input_embeds_1 = self.get_embed_tokens(cur_input_ids[:half_len])
                cur_input_embeds_2 = self.get_embed_tokens(cur_input_ids[half_len:])
            ...
            while X_token_indices.numel() > 0:
                ...
                cur_new_input_embeds.append(self.get_embed_tokens(cur_input_ids[:X_token_start])) 
            ...
            if cur_input_ids.numel() > 0:
                cur_new_input_embeds.append(self.get_embed_tokens(cur_input_ids))
                if labels is not None:
    ...
    def initialize_MM_tokenizer(self, model_args, tokenizer):
        ...
        if model_args.mm_use_time_token:
            num_new_tokens = tokenizer.num_new_tokens
            if num_new_tokens:
                self.resize_token_embeddings(len(tokenizer))
